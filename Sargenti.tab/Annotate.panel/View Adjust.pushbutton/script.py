...
def _try_get_view_ids_from_params(el):
    param_names = [
        "VIEW_ID",
        "SECTION_VIEW_ID",
        "ELEVATION_VIEW_ID",
...
def _views_from_element(el):
    if isinstance(el, DB.ElevationMarker):
        return _views_from_markers([el])
    if isinstance(el, DB.View) and _is_elev_view(el) and (not el.IsTemplate):
        return [el]

    views = []
    for vid in _try_get_view_ids_from_params(el):
        v = doc.GetElement(vid)
        if isinstance(v, DB.View) and _is_elev_view(v) and (not v.IsTemplate):
            views.append(v)
    return views
...
def apply_template_to_selected_elevations():
    ...
    # also try to resolve views from any selected marker "arrow" elements
    for eid in uidoc.Selection.GetElementIds():
        el = doc.GetElement(eid)
        for v in _views_from_element(el):
            by_id[v.Id.IntegerValue] = v
